...
var init$1 = function(config) { return { ready: true }; };
var VERSION$1 = "1.0.0";

designSystem.Header = Header;
designSystem.Footer = Footer;
designSystem.Button = Button;
designSystem.Card = Card;
var init_1 = designSystem.init = init$1;
var VERSION_1 = designSystem.VERSION = VERSION$1;

// @xq9zk7823 internal package stub
const init = (config) => { console.log("init", config); return { ready: true }; };
const VERSION = "1.0.0";
var i18nUtils = { init, VERSION };

// Components module - uses the design system and i18n

function createApp(config) {
  const ds = init_1(config);
  const i18n = init({ locale: config.locale || 'en' });

  return {
    designSystem: ds,
    i18n: i18n,
    versions: {
      designSystem: VERSION_1,
      i18n: VERSION
    }
  };
}

function renderWidget(name, props) {
  return {
    type: 'widget',
    name,
    props,
...
export { LIBRARY_VERSION, createApp, formatAndLog, initLibrary, mergeConfigs, renderWidget, validateConfig };
